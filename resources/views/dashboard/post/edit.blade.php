...
<div class="col-lg-8">
    <form method="POST" action="/dashboard/posts/{{ $post->id }}" class="mb-5" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          required autofocus value="{{ old('excerpt', $post->excerpt) }}">
        </div>
        <div class="mb-3">
          <label for="body" class="form-label">Body</label>
          <textarea rows="8" class="form-control @error('body') is-invalid @enderror" id="body" name='body' required autofocus>{{ old('body',$post->body) }}</textarea>
      </div>
